...
`package.json` infers `nodejs`, `tsconfig.json` infers `typescript`, and the
pnpm lock/workspace markers infer `pnpm`. Detection reads the root entry names
once without opening those files, traversing subdirectories, or sending match
evidence. Explicit tags come first and automatic tags supplement them only up
to the 32-tag payload limit.

The timestamp is generated by the CLI at send time.

## Opencode Plugin

Opencode does not expose a native hooks config equivalent to Codex Stop hooks,
but it does load JavaScript/TypeScript plugins that can subscribe to session
...
```txt
~/.config/opencode/plugins/clankerlog.ts
```

Restart Opencode after installing so it loads the plugin. The plugin listens for
`message.updated` events to remember the latest assistant message metadata for a
session, then listens for `session.idle` and sends one quiet clank with:

```txt
clankerlog ping --agent opencode --model <active Opencode model>
```

The plugin uses Opencode metadata for the session workspace and model. It does
not read or forward assistant text, prompts, source code, diffs, terminal
output, transcripts, or secrets.
